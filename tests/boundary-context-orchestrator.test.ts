/**
 * BoundaryContext Orchestrator — Integration Tests
...
describe("BoundaryContextOrchestrator", () => {
  let orchestrator: BoundaryContextOrchestrator;
  ...
  beforeEach(() => {
    storage = new InMemoryBoundaryContextStorage();
    eventBus = new BoundaryContextEventBus();
    orchestrator = new BoundaryContextOrchestrator(config, storage, eventBus);
  });

  it("should initialize in IDLE state", () => {
    expect(orchestrator.getState()).toBe(BoundaryContextState.IDLE);
  });
  ...
  it("should return valid telemetry", () => {
    ...
    expect(telemetry.state).toBe(BoundaryContextState.IDLE);
  });
